...
class HurricaneDNS(object):
    ...
    def add_domain(self, domain, main=None, method=None):
        domain = domain.lower()
        data = {
            'retmain': 0,
            'submit': 1
        ...
        if main and method:
            raise HurricaneBadArgumentError('Domain "%s" can not be both subordinate and reverse' % domain)

        if main:
            if isinstance(main, list) or isinstance(main, tuple):
                i = 1
                for ns in main:
                    data['main%s' % i] = ns
                    i += 1
                ...
                data['main1'] = main
            data['add_subordinate'] = domain
            data['action'] = 'add_subordinate'
        elif method:
    ...
    def __add_or_edit_record(self, domain, record_id, host, rtype, value, mx, ttl):
        ...
        if d['type'] == 'subordinate':
            raise HurricaneBadArgumentError('Domain "%s" is a subordinate zone, this is a bad idea!' % domain)
    ...
    def list_records(self, domain):
        ...
        elif d['type'] == 'subordinate':
            res = self.__process({
                'domid': d['id'],
                'menu': 'edit_subordinate',
                'action': 'edit'
            })
