...
@api
def create(domain_name=None, zone_type='main', ns=[], main_ip=None):
    """Creates a new DNS zone.

    :param domain_name: string, (required) the domain name for which to
        create the zone
    :param zone_type: string, (required) mastor, subordinate, parked, or geodns
    :param ns: list (optional) list of nameservers used for starting ns
        record; only for main domains
    :param main_ip: string (optional, required for subordinate domains) main
        server IP address
    """
    ...
    if hasattr(zone_type, 'lower') and zone_type.lower() == 'subordinate':
        param_args['main-ip'] = {
            'value': main_ip,
            'optional': False,
        }

    if hasattr(zone_type, 'lower') and zone_type.lower() == 'main':
        param_args['ns'] = {
            'value':    ns,
            'optional': True,
        }
